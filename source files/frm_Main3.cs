...
/// <summary>
/// This version is a rewrite to compensate for third party NIC drivers. In this case
/// for Broadcom gigabit ethernet drivers which don't support some WMI methods.
/// </summary>
...
namespace FileRenamer3
{
	public partial class frm_Main3 : Form
	{
		...
		Timer tTime;
		clsDiskChangeAlerter3 DAlerter;
		bool bDriveRescan = false;
		GettingDrivesNotice3 frmNotice = new GettingDrivesNotice3();
		clsFileChangeWatcher3 FileWatcher;
		...
		/// <summary>
		/// delay to let program settle before trying to read drives and folders
		/// </summary>
		...
		private int GetDrives()
		{
			...
			string sCurDriveText = "", sDriveName = "";
			int iDriveType = 0;
			...
			try
			{
				...
				{
					iDriveType = (int)oInfo.DriveType;
									...
									|| oInfo.DriveType == DriveType.Removable)
					{
						...
						{
							if (iDriveType == 4)
								sDriveName = "Network Drive";
							else
							{
								sDriveName = oInfo.VolumeLabel;
								if (DAlerter.UsbDriveList.Contains(oInfo.Name))
								{ sDriveName += " {USB}"; }
							}
							cbDriveList.Items.Add(oInfo.Name + " - " + sDriveName);
							if (oInfo.Name.Contains(sCurDrive))
								sCurDriveText = oInfo.Name + " - " + sDriveName;
						}
						else
						{
		...
		{
			// todo: rewrite usbdrives check changed

			GetDrives(); // do the steering in that method
